...
4. **Given** 用户 A 已提交加入申请，**When** A 查询自己的申请，**Then** A 能看到当前状态（待审批/已通过）
...
5. owner 同意 → 申请人成为 member，系统尽力推送通知申请人「已加入」（48h 内可推则推，推不了入 undelivered；申请人下次对话时补发提示）
...
5. **Given** owner 同意申请，**When** 申请人成为 member，**Then** 收到加入通知（推送或下次对话补发），且当前账本不自动切换，需自行切换
...
3. **Given** 用户仅有一个账本，**When** 发起管理操作，**Then** 直接执行，无需确认
...
3. **Given** 账本有未审批的加入申请，**When** owner 重置口令，**Then** 这些申请全部作废（状态置 `expired`），不再出现在待审批列表
...
- **owner 在任何情况下不可被移除、不可退出**（单 owner 模型，只能删账本）。
...
- **FR-015**: 系统 MUST 让申请人能查询自己的申请状态（待审批 / 已通过）。
- **FR-016**: owner 同意后，系统 MUST 尽力通知申请人（48h 窗口内推送；失败入 undelivered 队列，下次对话补发提示）。
...
- **SC-007**: 查账结果中，可显示记账人昵称的账目比例 = 100%（存量 NULL 已清理）。
...
- 移除/退出不改变账目归属；存量 `ledger_id`/`created_by_user_id` 为 NULL 的账目（含『历史账本』）经确认为测试数据，本期一次性清理删除（见前置清理任务）。
- 本期不实现「拒绝申请」与「申请自动过期」：owner 对不想批准的申请无法主动清除，待审批列表只增不减（唯一出路是「同意」）。`rejected` 为预留状态值；`expired` 仅用于「口令重置时作废旧申请」。拒绝与自动过期列为后续增强。
